...
namespace FAE
{
    public class FAE_Core : Editor
    {
        ...
        public static string INSTALLED_VERSION = "1.5.0";
                ...
                for (int i = 0; i < mats.Length; i++)
                {
                    ...
                    if (matched)
                    {
                        if (source.Contains("Particle") && !source.Contains("Leaf"))
                        {
                            ...
                            if (mats[i].HasProperty("_Surface")) mats[i].SetFloat("_Surface", 1);
                            //Additive blending
                            if (mats[i].HasProperty("_ColorMode")) mats[i].SetFloat("_ColorMode", 1);
                        }
                        ...
                        if (config == ShaderInstallation.UniversalRP && mats[i].HasProperty("_MainTex"))
                        {
                            ...
                            if (mats[i].HasProperty("_MainTex")) mats[i].SetTexture("_BaseMap", mats[i].GetTexture("_MainTex"));
                        }
