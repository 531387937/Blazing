...
#pragma warning restore 649
        ...
        protected virtual void InitializedApplyDeltaTime(float deltaTime)
        {
            ...
            {
                float speed = UseOffset && OffsetCompensation
            ...
            Vector3 preRefreshPosition = Transform.position;
            Quaternion preRefreshOrientation = Transform.rotation;
            ...
            if (DirectionDampingTime > 0 && State == CurvyControllerState.Playing)
            {
                postDampingForward = deltaTime > 0
                    ? Vector3.SmoothDamp(Transform.forward, newForward, ref DirectionDampingVelocity, DirectionDampingTime, float.PositiveInfinity, deltaTime)
                    : Transform.forward;
            }
            else
            ...
            if (UpDampingTime > 0 && State == CurvyControllerState.Playing)
            {
                postDampingUp = deltaTime > 0
                    ? Vector3.SmoothDamp(Transform.up, newUp, ref UpDampingVelocity, UpDampingTime, float.PositiveInfinity, deltaTime)
                    : Transform.up;
            }
            else
                postDampingUp = newUp;
            ...
            Transform.rotation = Quaternion.LookRotation(postDampingForward, postDampingUp);
            Transform.position = newPosition;

            if (preRefreshPosition.NotApproximately(Transform.position) || preRefreshOrientation.DifferentOrientation(Transform.rotation))
                UserAfterUpdate();
        }
        ...
        /// <summary>
        /// Retrieve the source global, tangent and orientation for a given relative position (TF)
        /// </summary>
        ...
        {
            Quaternion R = Quaternion.AngleAxis(offsetAngle, tangent);
            return position + (R * up) * offsetRadius;
        }
        ...
        protected float ComputeOffsetCompensatedSpeed(float deltaTime)
        {
            ...
            if (CurvyUtility.Approximately(OffsetRadius, 0))
                return Speed;
