...
#pragma warning restore 618
        ...
        void doUpdate()
        {
            ...
            if (transform.hasChanged)
            {
                ...
                if (transform.position.NotApproximately(lastProcessedPosition) || transform.rotation.DifferentOrientation(lastProcessedRotation) || transform.lossyScale != lastProcessedScale)
                {
        ...
        {
            var names = new string[CachedControlPointsNameCount];
        ...
        {
#if CONTRACTS_FULL
            Contract.Requires(controlPoint != null);
        ...
        {
            return (short)((controlPointIndex + 1 < controlPointsCount) ? controlPointIndex + 1 : (isSplineClosed ? 0 : -1));
        }

        static private short GetPreviousControlPointIndex(short controlPointIndex, bool isSplineClosed, int controlPointsCount)
        {
            return (short)((controlPointIndex - 1 >= 0) ? controlPointIndex - 1 : (isSplineClosed ? controlPointsCount - 1 : -1));
        }
...
#endif
            return isClosed || (notAutoEndTangentsAndIsCatmullRomOrTCB
        ...
        /// </summary>
        /// <param name="followUp">The Control Point set as "Head To" in the connection inspector</param>
        /// <param name="headToDirection">The head to direction</param>
        /// <returns></returns>
        [NotNull]
        ...
        {
#if CONTRACTS_FULL
            Contract.Requires(followUp != null);
            Contract.Requires(followUp.Spline != null);
            Contract.Ensures(Contract.Result<CurvySplineSegment>() != null);
...
#endif
            ConnectionHeadingEnum resolveHeading = headToDirection.ResolveAuto(followUp);
        ...
        {
#if CONTRACTS_FULL
            Contract.Requires(controlPoints.Contains(item));
            Contract.Ensures(mDirtyCurve);
#endif
            ...
            lock (controlPointsRelationshipCacheLock)
            {
                if (isCpsRelationshipCacheValid == false)
                {
                    ...
                    if (controlPointsCount > 0)
                    {
                        ...
                        lastVisibleControlPoint = lastSegment != null
            ...
            if (dirtyCpsExtendedList.Count > 0)
            {
                ...
                if (mDirtyCurve)
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dirtyCpsExtendedList.Count; i++)
                        {
                            CurvySplineSegment dirtyCp = dirtyCpsExtendedList[i];
                            dirtyCp.refreshCurveINTERNAL(interpolation, IsControlPointASegment(dirtyCp), this);
                        }
                    }
...
#pragma warning restore 618
        ...
        /// <summary>
        /// Prepare the TTransform for all needed control points
        /// </summary>
        private void PrepareTTransforms()
        {
            ...
            for (int i = 0; i < controlPointsCount; i++)
            {
                ...
                controlPoint.PrepareThreadSafeTransfromINTERNAL();
            }
            ...
            if (Count > 0)
            {
                ...
                if (beforeFirst && beforeFirst.Spline != this)
                    beforeFirst.PrepareThreadSafeTransfromINTERNAL();
                ...
                if (afterLast && afterLast.Spline != this)
                    afterLast.PrepareThreadSafeTransfromINTERNAL();
            }
        }
        ...
        {
            lastProcessedPosition = transform.position;
            lastProcessedRotation = transform.rotation;
            lastProcessedScale = transform.lossyScale;
        }
        ...
        private CurvySplineSegment InsertAt(CurvySplineSegment controlPoint, Vector3 globalPosition, int insertionIndex, CurvyControlPointEventArgs.ModeEnum insertionMode, bool skipRefreshingAndEvents)
        {
            ...
            if (UsePooling && Application.isPlaying)
            {
                ...
                if (curvyGlobalManager != null)
                {
                    insertedControlPoint = curvyGlobalManager.ControlPointPool.Pop<CurvySplineSegment>(transform);//TODO should this be callse with "null" instead of "transform", to be coherent with the other branches, and knowing that the parent is set to "transform" anyway in subsequent calls?
            ...
            insertedControlPoint.transform.position = globalPosition;
