...
#region --- CG Options ---
        ...
        */
        ...
        /// <summary>
        /// This exists because Transform can not be accesed in non main threads. So before refreshing the spline, we store the local position here so it can be accessed in multithread spline refreshing code
        /// </summary>
        ...
        float mStepSize;
        Bounds? mBounds;
        int mCacheLastDistanceToLocalFIndex;

        /// <summary>
        /// The Metadata components added to this GameObject
        /// </summary>
        private readonly HashSet<Component> mMetaData = new HashSet<Component>(); //TODO make this a set of CurvyMetadataBase once you finish the refactoring
        /// <summary>
        ...
        void OnDrawGizmos()
        {
            if (Spline && Spline.ShowGizmos)
            {
                ...
                while (!willOnDrawGizmosSelectedGetCalled && testedTransform != null);
        ...
        internal void refreshCurveINTERNAL(CurvyInterpolation splineInterpolation, bool isControlPointASegment, CurvySpline spline)
        {
            ...
            CurvySplineSegment nextControlPoint = nextControlPointIndex == -1
                ? null
                : spline.ControlPointsList[nextControlPointIndex];

            int newCacheSize;
            if (isSegment)
            {
#if CURVY_SANITY_CHECKS
                Assert.IsNotNull(nextControlPoint);
#endif
                newCacheSize = CurvySpline.CalculateCacheSize(
                    spline.CacheDensity,
                    (nextControlPoint.threadSafeLocalPosition - threadSafeLocalPosition).magnitude,
            ...
            Array.Resize<Vector3>(ref Approximation, newCacheSize + 1);
            Array.Resize<Vector3>(ref ApproximationT, newCacheSize + 1);
            Array.Resize<float>(ref ApproximationDistances, newCacheSize + 1);
            Array.Resize<Vector3>(ref ApproximationUp, newCacheSize + 1);
            ...
            if (newCacheSize != 0)
            {
                Approximation[newCacheSize] = nextControlPointIndex != -1
                    ? nextControlPoint.threadSafeLocalPosition
            ...
            if (isSegment)
            {
                ...
                switch (splineInterpolation)
                {
                    case CurvyInterpolation.Bezier:
                        segmentLength = InterpolateBezierSegment(spline, newCacheSize);
                        break;
                    case CurvyInterpolation.CatmullRom:
                        segmentLength = InterpolateCatmullSegment(spline, nextControlPoint, newCacheSize);
                        break;
                    case CurvyInterpolation.TCB:
                        segmentLength = InterpolateTCBSegment(spline, nextControlPoint, newCacheSize);
                        break;
                    case CurvyInterpolation.Linear:
                        segmentLength = InterpolateLinearSegment(spline, newCacheSize);
                        break;
                    default:
            ...
            else
            {
                if (nextControlPointIndex != -1)
                    ApproximationT[0] = (nextControlPoint.threadSafeLocalPosition - Approximation[0]).normalized;
                else
                {
        ...
        private float InterpolateBezierSegment(CurvySpline spline, int newCacheSize)
        {
            ...
            CurvySplineSegment ncp = spline.GetNextControlPoint(this);
        ...
        private float InterpolateTCBSegment(CurvySpline spline, CurvySplineSegment nextControlPoint, int newCacheSize)
        {
            ...
            if (!OverrideGlobalTension)
                ft0 = ft1 = spline.Tension;
            if (!OverrideGlobalContinuity)
                fc0 = fc1 = spline.Continuity;
            if (!OverrideGlobalBias)
                fb0 = fb1 = spline.Bias;


            CurvySplineSegment previousControlPoint = spline.GetPreviousControlPointUsingFollowUp(this);
            CurvySplineSegment nextNextControlPoint = nextControlPoint.Spline.GetNextControlPointUsingFollowUp(nextControlPoint);

            Vector3 p0 = threadSafeLocalPosition;
            Vector3 p1 = nextControlPoint.threadSafeLocalPosition;
            Vector3 t0 = previousControlPoint
                ? previousControlPoint.threadSafeLocalPosition
                : p0;
            Vector3 t1 = nextNextControlPoint
                ? nextNextControlPoint.threadSafeLocalPosition
                : p1;
        ...
        private float InterpolateCatmullSegment(CurvySpline spline, CurvySplineSegment nextControlPoint, int newCacheSize)
        {
            ...
            //OPTIM I bet ou can get significant perfs increase by using GetNextControlPointIndex (and similar) instead of GetNextControlPoint (and similar)
            float lengthAccumulator = 0;
            CurvySplineSegment previousControlPoint = spline.GetPreviousControlPointUsingFollowUp(this);
            CurvySplineSegment nextNextControlPoint = nextControlPoint.Spline.GetNextControlPointUsingFollowUp(nextControlPoint);
        ...
        private float InterpolateLinearSegment(CurvySpline spline, int newCacheSize)
        {
            ...
            Vector3 pEnd = spline.GetNextControlPoint(this).threadSafeLocalPosition;
            Vector3 tangent;
            for (int i = 1; i < newCacheSize; i++)
            {
                float localF = i * mStepSize;
                Approximation[i] = Vector3.LerpUnclamped(pStart, pEnd, localF);
        ...
        void doGizmos(bool selected)
        {
            ...
            if (!Spline.IsControlPointASegment(this)) return;
        ...
        {
            CurvySpline curvySpline = Spline;
            if (stepSize == 0 || Mathf.Approximately(newSplineLength, curvySpline.Length)) return true;
            Transform cachedTransform = transform;

            float curLength = curvySpline.Length;
            Vector3 oldPos = cachedTransform.position;
            Vector3 upstep = cachedTransform.up * stepSize;

            // Check if increasing by Up-Vector will increase the length
            cachedTransform.position += upstep;
            curvySpline.SetDirty(this, SplineDirtyingType.Everything);
            curvySpline.Refresh();
            bool UpGrows = (curvySpline.Length > curLength);
            int loops = 30000;
            cachedTransform.position = oldPos;
            ...
            if (newSplineLength > curLength)
            {
                if (!UpGrows)
                    upstep *= -1;
                while (curvySpline.Length < newSplineLength)
                {
                    loops--;
                    curLength = curvySpline.Length;
                    cachedTransform.position += upstep;
                    curvySpline.SetDirty(this, SplineDirtyingType.Everything);
                    curvySpline.Refresh();
                    if (curLength > curvySpline.Length)
                    {
                        return false;
                    }
                    if (loops == 0)
        ...
        /// <summary>
        /// Set the correct values to the thread safe local position and rotation
        /// </summary>
