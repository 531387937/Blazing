...
namespace FluffyUnderware.CurvyEditor.UI
{
    ...
    public class TBDrawControlPoints : DTToolbarToggleButton
    {
        ...
        void addCP(Vector2 cursor, bool castRay, bool connectNew)
        {
            Func<CurvySpline, CurvySplineSegment, Vector3, CurvySplineSegment> insertControlPoint =
                ...
                {
                    CurvySplineSegment seg = spline.InsertAfter(current, worldPos, false);
            ...
            if (selSpline)
            {
                if (!selCP && selSpline.ControlPointCount > 0)
                    selCP = selSpline.ControlPointsList[selSpline.ControlPointCount - 1];
            }
            else
            {
            ...
            if (connectNew && selCP)
            {
                ...
                if (gameObjectUnderCursor)
                {
                    ...
                    if (cpUnderCursor)
                    {
                        Plane P = new Plane(SceneView.currentDrawingSceneView.camera.transform.forward, cpUnderCursor.transform.position);
                        Ray R = HandleUtility.GUIPointToWorldRay(cursor);
                        float dist;
                        if (P.Raycast(R, out dist))
                        {
                            //Setting connectedCp
                            {
                                ...
                                {
                                    Vector3 position = cpUnderCursor.Spline.transform.localToWorldMatrix.MultiplyPoint3x4(
                                        cpUnderCursor.Interpolate(cpUnderCursor.GetNearestPointF(hit))
                                        );
                                    cpToConnect = cpUnderCursor.Spline.InsertAfter(cpUnderCursor, position, false);
                                }
                            }

                            newCP = insertControlPoint(selSpline, selCP, cpToConnect.transform.position);
                            selCP = newCP;
                        }
                        else
                ...
                if (!cpToConnect)
                {
                    CurvySpline newSpline = CurvySpline.Create(selSpline);
                    newSpline.Closed = false;
                    cpToConnect = insertControlPoint(newSpline, null, selCP.transform.position);
...

                {
                    ...
                    Assert.IsFalse((selCP.Connection != cpToConnect.Connection) && (selCP.Connection != null && cpToConnect.Connection != null), "Both Control Points should not have different non null connections");
#endif
                    CurvyConnection connection;
                    {
                        connection = selCP.Connection;
                        if (connection != null)
                        {
                            cpToConnect.ConnectionSyncPosition = selCP.ConnectionSyncPosition;
                            cpToConnect.ConnectionSyncRotation = selCP.ConnectionSyncRotation;
                            cpToConnect.FollowUpHeading = selCP.FollowUpHeading;
                        }
                        else
                        {
                            connection = cpToConnect.Connection;
                            if (connection != null)
                            {
                                selCP.ConnectionSyncPosition = cpToConnect.ConnectionSyncPosition;
                                selCP.ConnectionSyncRotation = cpToConnect.ConnectionSyncRotation;
                                selCP.FollowUpHeading = cpToConnect.FollowUpHeading;
                            }
                            else
                            {
                                connection = CurvyConnection.Create(selCP, cpToConnect);
                                selCP.ConnectionSyncPosition = cpToConnect.ConnectionSyncPosition = true;
                                selCP.ConnectionSyncRotation = cpToConnect.ConnectionSyncRotation = true;
                                selCP.FollowUpHeading = cpToConnect.FollowUpHeading = ConnectionHeadingEnum.Auto;
                            }
                        }
                    }


                    if (connection.ControlPointsList.Contains(selCP) == false)
                        connection.AddControlPoints(selCP);
                    if (connection.ControlPointsList.Contains(cpToConnect) == false)
                        connection.AddControlPoints(cpToConnect);

                    connection.SetSynchronisationPositionAndRotation(cpToConnect.transform.position, cpToConnect.transform.rotation);
                }
            }
            else
            {
                newCP = insertControlPoint(selSpline, selCP, pos);
            }
    ...
    public class TBCPNextConnected : DTToolbarButton
    {
        ...
        public override void OnClick()
        {
            ...
            if (cp)
            {
                var idx = (int)Mathf.Repeat(cp.Connection.ControlPointsList.IndexOf(cp) + 1, cp.Connection.ControlPointsList.Count);
    ...
    public class TBCPJoin : DTToolbarButton
    {
        ...
        public override void OnClick()
        {
            ...
            var selIdx = destCP.Spline.GetControlPointIndex(destCP) + source.ControlPointCount + 1;
