        ...
        void showSlots<T>(List<T> slots) where T:CGModuleSlot
        {
            foreach (CGModuleSlot slot in slots)
            {
                ...
                if (linked.Count > 1)
                {
                    ...
                    {
                        var sel = EditorGUILayout.ObjectField((i==0)?slot.Info.DisplayName : " ", linked[i], typeof(CGModule), true);
                        if (sel!=linked[i])
                ...
                else
                {
                    ...
                    var sel=EditorGUILayout.ObjectField(slot.Info.DisplayName, lm, typeof(CGModule), true);
                    if (sel != lm)
                        DTLog.Log("[Curvy] Linking modules from the inspector isn't supported yet! Use the Graph editor instead!");
                }
            }
