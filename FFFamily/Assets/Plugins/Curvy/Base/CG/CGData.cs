...
namespace FluffyUnderware.Curvy.Generator
{
    ...
    public class CGShape : CGData
    {
        ...
        */
        ...
        public Vector3 InterpolatePosition(float f)
        {
            ...
            return Vector3.LerpUnclamped(Position[idx], Position[idx + 1], frag);
        }
        ...
        public void Interpolate(float f, out Vector3 position, out Vector3 up)
        {
            float frag;
            int idx = GetFIndex(f, out frag);
            position = Vector3.LerpUnclamped(Position[idx], Position[idx + 1], frag);
        ...
        public void RecalculateNormals(List<int> softEdges)
        {
            if (Normal.Length != Position.Length)
                Array.Resize<Vector3>(ref Normal, Position.Length);
    ...
    public class CGPath : CGShape
    {
        ...
        //TODO change method's name or content, recalculate is confusing, since it does not calculate everything, and API user is left to guess what is recalculated, and what is not
        //DESIGN Recalculate shound not be overridable, and Direction should be computed in another method. The issue with the current state of affaires is that someone calling Recalculate on a CGPath through a reference of type CGData will think that he/she only updates the Length and F properties
        /// <summary>
        /// Recalculate Length and F[] (by measuring a polyline built from all Position points) and Direction
        /// </summary>
        public override void Recalculate()
        {
            base.Recalculate();
            //BUG does not recalculate Direction[0]
            for (int i = 1; i < Count; i++)
            {
                //Inlined version of Direction[i] = (Position[i] - Position[i - 1]).normalized;
    ...
    public class CGVMesh : CGBounds
    {
        ...
        public void MergeVMeshes(List<CGVMesh> vMeshes, int startIndex, int endIndex)
        {
            ...
            foreach (var pair in submeshesByMaterial)
            {
    ...
    public class CGDataRequestShapeRasterization : CGDataRequestRasterization
    {
        ...
        public override bool Equals(object obj)
        {
            var other = obj as CGDataRequestShapeRasterization;
            if (other == null)
                return false;
    ...
    public class CGDataRequestRasterization : CGDataRequestParameter
    {
        ...
        /// <summary>
        /// The length, expressed as a distance in world units, of the spline in its fullest.
        /// </summary>
